...
def create_account(num, type, name, init_balance): #สร้าง account
    index = search_account_db(num) # ค้นหา แอคด้วย เลขแอค
    if index == -1: # ตรงนี้คือตรวจสอบว่าโอเค ยังไม่มีแอคนะ
        account = {} # เก็บไว้ใน dict ก่อน (เป็นของแต่ละคน)
        account["account_number"] = num
        account["type"] = type
        account["account_name"] = name
        account["balance"] = init_balance
        account_database.append(account) # แล้วเก็บไว้ใน list
    else:
        print("Account", num, "already exists") # กรณีที่มีแอคอยู่แล้ว


def delete_account(num): # ลบบัญชี
    index = search_account_db(num) # ค้นหา แอคด้วย เลขแอค
    if index != -1: #กรณีนี้คือมันมีแอคอยู่แล้ว (index ตรงนี้จะเท่ากับ i ไม่ได้เท่ากับ -1)
...
def search_account_db(num):
    for i in range(len(account_database)): #loop ตามจำนวน แอคที่มีอยู่ใน database
        if account_database[i]["account_number"] == num: #ถ้า มีแอคอยู่แล้ว
            return i  #ให้ return i ออกไป
    return -1 #แต่ถ้ายังไม่มีแอค ให้ return -1 ออกไป
...
def withdraw(account_num, amount):
    ...
    if index != -1:
        if account_database[index]["balance"] >= amount:
            ...
            print("withdrawal amount", amount, "exceeds the balance of", account_database[index]["balance"], "for",
                  account_num, "account.")
    else:
