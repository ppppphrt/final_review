...
# test = my_table1.filter(lambda x : x['city'] == 'Aalborg')
# print(test)
...
my_filtered_table = my_table3.filter(
    lambda x: 'ia' in x['team'] and float(x['minutes']) < 200.0 and float(x['passes']) > 100.0)
...
my_pivot = my_table5.pivot_table(['class', 'gender', 'survived'], ['survived', 'fare'],
